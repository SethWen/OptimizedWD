...
// 根据 sessionId 获取 WebDriver 实例
Webdriver.prototype._initFromSessionId = function() {
  var _this = this;
  var fargs = utils.varargs(arguments);
  var cb = fargs.callback;
  _this.sessionID = fargs.all[0];

  if (_this.sessionID) {
    // http options init
      var httpOpts = httpUtils.newHttpOpts('GET', _this._httpConfig);

      var url = httpUtils.buildJsonCallUrl(_this.noAuthConfigUrl, _this.sessionID);

      // logging
      httpUtils.emit(_this, httpOpts.method, url);

      // writting data
      httpOpts.prepareToSend(url);
      // building request
      httpUtils.requestWithRetry(httpOpts, _this._httpConfig, _this.emit.bind(_this), function(err, res, data) {
          if(err) { return cb(err); }

          var jsonData = JSON.parse(strip(data));
          var status = jsonData.status;
          if (status === 0) {
            cb(null, strip(data));
          } else if (status === 6) {
            return cb(new Error('A session is either terminated or not started'));
          } else {
            return cb(new Error('Unknown error'));
          }
      });
  } else {
    console.error('You have not deliver sessionId');
    return cb(new Error('You have not deliver sessionId'));
  }
};
